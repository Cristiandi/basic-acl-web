<script>
    ...
    async function loadRolesList() {
      ...
      return data.map(item => ({ value: item.id, label: `${item.code} - ${item.name}` }));
    }
    ...
    <div slot="content">
      <form name="form" on:submit|preventDefault={handleSubmitCreate}>
        <div class="form-group">
          ...
          <Select name="role" items={rolesList} bind:selectedValue={current.role} />
          {#if errors.roleId}<span class="validation">{errors.roleId}</span>{/if}
        </div>
        <div class="form-group">
          <label for="user">User</label>
          <Select name="user" items={usersList} bind:selectedValue={current.user} />
          {#if errors.userId}<span class="validation">{errors.userId}</span>{/if}
        </div>
        <div class="form-group">
          <label for="apiKey">Api key</label>
          <Select name="apiKey" items={apiKeysList} bind:selectedValue={current.apiKey} />
          {#if errors.apiKeyId}<span class="validation">{errors.apiKeyId}</span>{/if}
        </div>
        <div class="form-group">
